...
export function CreateProductSubCategoryDrowpdown({
  value,
  onChange,
  selectedCategoryId,
}: SubCategoryDrowpdownProps) {
  ...
  const createSubCategory = React.useCallback(
    async (name: string, categoryId: string) => {
      try {
        if (!categoryId) {
          toast.error(
            "Ingen kategori vald, du måste välja kategori för att kunna skapa en underkategori",
          );
          return;
        }
        ...
        if (!response.ok) {
          ...
          throw new Error(
            error.error || "Misslyckades att skapa underkategori",
          );
        }
        ...
        const categoryProducts = products?.filter(
          (product) =>
            product.categoryId === categoryId && product.subCategoryId === null,
        );
          ...
          await Promise.all(
            categoryProducts?.map(async (product) => {
              ...
              if (!productResponse) {
                ...
                throw new Error(
                  error.error ||
                    "Misslyckades att flytta produkter till ny underkategori",
                );
              }
            }),
          );
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Misslyckades att skapa underkategori",
        );
      } finally {
      }
    },
