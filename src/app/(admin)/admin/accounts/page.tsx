...
export default function Accounts() {
  const [selectedUserInvite, setSelectedUserInvite] = useState<UserInvite | null>(null);
  ...
  const filteredInvites = userInvites?.filter((userInvite) => !userInvite.publicMetadata.notificationSent) ?? [];
  const sentInvites = userInvites?.filter((userInvite) => userInvite.publicMetadata.notificationSent) ?? [];
  ...
  const handleRevokeInvite = async (userInviteId: string) => {
    ...
    const confirmed = window.confirm("Är du säker på att du vill radera inbjudan? Detta går inte att ångra.");
    if (!confirmed) return;
    try {
  ...
  return (
    <>
        ...
        <div className="my-4">
          {
            isLoading ? <Loader2 className="h-6 w-6 animate-spin" />
              ...
              <div className="border border-secondary/50 rounded-lg flex flex-col gap-2 shadow-lg">
                <h3 className="bg-secondary text-background rounded-t-md px-2 py-1 shadow-lg">Inbjudningar</h3>
                <div className="px-2">
                    ...
                    <ul>
                      {filteredInvites.map((invite, i) => (
                        <li key={i} className="flex justify-between">
                          ...
                          >
                            {loading === invite.emailAddress && <Loader2 className="h-4 w-4 animate-spin" />}
      ...
      {newUserInvite &&
        <UserDialog open={open} onOpenChange={setOpen} user={newUserInvite} action={"sendInvite"} />
      }
    </>
  )
}
