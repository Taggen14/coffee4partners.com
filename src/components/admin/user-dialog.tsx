...
export function UserDialog({
  ...
  async function handleDeleteUser(userId: string) {
    const confirmed = window.confirm(
      "Är du säker på att du vill radera kontot? Detta går inte att ångra.",
    );
    if (!confirmed) return;
    try {
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <p className="text-xs pb-2">
                        Admin kommer få full åtkomst till att skapa och ändra
                        produkter i webbshoppen samt hantera ordrar. Kund kommer
                        åt checkout och kan lägga en beställning.
                      </p>
                      <FormControl>
                        <RadioGroup
